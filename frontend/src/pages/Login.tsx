...
export default function Login() {
  ...
  const handleSocialLogin = (provider: "Google" | "GitHub") => {
    if (role !== "STUDENT") {
      setStatus("error");
      setErrorMessage("Social login is only available for student accounts.");
      return;
    }
    setStatus("loading");
    const redirectUri = encodeURIComponent("http://localhost:5173/login");
    const state = `student_${provider.toLowerCase()}`;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (role === "ADMIN" && view === "signup") {
        throw new Error("Admin accounts cannot be registered publicly.");
      }
      if (role === "PANELIST" && view === "signup") {
        throw new Error(
          "Panelist accounts are created and validated by Admin.",
        );
      }

      if (view === "login") {
        validateForm();
        await requestLoginOtp({ email, password, role: role.toLowerCase() });
        setOtpContext("login");
        setView("otp");
        setStatus("idle");
      } else if (view === "signup") {
        ...
        <div className="w-full max-w-[420px]">
          <header className="flex flex-col items-center mb-8 text-center animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <p className="text-slate-500 dark:text-slate-400 text-sm mt-1.5 font-medium">
              {view === "login" &&
                `Accessing ${role.charAt(0) + role.slice(1).toLowerCase()} workspace.`}
              {view === "signup" && "Register your student profile."}
          ...
          <div className="bg-white dark:bg-[#0A0A12] border border-slate-200 dark:border-white/10 rounded-[2rem] p-6 sm:p-8 shadow-xl shadow-slate-900/5 relative overflow-hidden animate-in fade-in zoom-in-95 duration-500">
            ...
            >
              ...
              {role === "PANELIST" && view === "login" && (
                <div className="p-3 bg-indigo-50 dark:bg-indigo-500/10 border border-indigo-200 dark:border-indigo-500/20 rounded-xl text-indigo-700 dark:text-indigo-300 text-xs font-semibold flex items-center gap-2">
                  <ShieldAlert className="w-4 h-4 shrink-0" />
                  <span>
                    Panelist accounts are created & validated by Admin.
                  </span>
                </div>
              )}

              {role === "ADMIN" && view === "login" && (
                <div className="p-3 bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/20 rounded-xl text-amber-800 dark:text-amber-300 text-xs font-semibold flex items-center gap-2">
                  <ShieldAlert className="w-4 h-4 shrink-0" />
                  <span>Single admin portal access active.</span>
                </div>
              )}
