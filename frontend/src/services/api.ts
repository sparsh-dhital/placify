...
const extractTextFromImage = async (file: File): Promise<string> => {
  ...
  try {
    ...
    enhancedCanvas.width = baseCanvas.width;
    enhancedCanvas.height = baseCanvas.height;
    const enhancedCtx = enhancedCanvas.getContext("2d");
    if (enhancedCtx) {
      enhancedCtx.drawImage(baseCanvas, 0, 0);
      candidates.push(preprocessCanvasImage(enhancedCanvas, false, 120));
      candidates.push(preprocessCanvasImage(enhancedCanvas, true, 160));
      candidates.push(preprocessCanvasImage(enhancedCanvas, false, 170));
    }
    ...
    const directText = (directResult.data.text || "")
      .replace(/\r/g, "\n")
      .split("")
      .filter((character) => character === "\n" || character === "\t" || character.charCodeAt(0) >= 32)
      .join("")
